...
void comserial::escreveSerial(QSerialPort *port, const QByteArray &writeData)
{
    //qDebug() << "TX: " << writeData;
     ...
     */
    if (bytesWritten == -1) {
        qDebug() << "Falha ao escrever os dados na porta.";
    } else if (bytesWritten != writeData.length()) {
        qDebug() << "Falha ao escrever uma parte dos dados na porta.";
    }
}

QByteArray comserial::recebeSerial(QSerialPort *port)
{
